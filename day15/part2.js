...
var readLine = R.compose(trace, R.map(tryParseInt), R.drop(2), ingredientRegex); 
var parseInput = R.compose(R.map(readLine), R.split('\n'), R.trim);
var getProperties = R.compose(R.keys, R.omit(['name']), R.head);

// var calc = (scores, indexes) => {
// 	for(var i = 0; i <= 100; i++) {
// 		if (R.sum(indexes) + i > 100) return;
		...
		for(var b = 0; b <= 100; b++) {
			...
			for(var c = 0; c <= 100; c++) {
				...
				for(var d = 0; d <= 100; d++) {
					...
					if (scores[4] !== 500) continue;
					
					var total = R.reduce(R.multiply, 1, R.dropLast(1, scores));
					if (total > max) {
						max = total;
					}
				}
